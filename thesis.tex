...
To handle conflicts and to trace changes, Ethermap provides a continuous history feed of all changes on a map.
...
A history management tool could help users understand changes and experiment more freely, by being able to revert changes.
...
To bridge that gap, the article introduces interactive features designed to balance collaboration and autonomy:
\begin{enumerate}
    \item \textbf{Peeking} Temporarily viewing another user’s state
    \item \textbf{Tracking} Synchronizing views with another user’s actions in real time.
    \item \textbf{Forking} Independent exploration starting from another user’s state)
\end{enumerate}
...
Litt et al. present Peritext, a Conflict-Free Replicated Data Types (CRDTs) algorithm extrended to rich text editing.\cite{peritext}
% TODO peritext summarize a bit more
