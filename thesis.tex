...
\begin{enumerate}
  ...
  \item \textbf{Correctness}: Is the order of actions that the client receives always in the same order as written in the database? 
\end{enumerate}
...
To move one or more plantings, the client makes an \gls{http} \texttt{PATCH} request to \urlsnippet{/api/maps/<map_id>/layers/plants/plantings} with the desired map identifier and the following \gls{json} payload.
...
\begin{enumerate}
    ...
    In PermaplanT any request that alters the map should finish the entire request, so that requests never interleave and cause inconsistency.
    What is interesting in our case is that the shared data is not in memory on the same computer but the order of network events that cause mutation in the client application's memory.

    
    In the existing implementation \texttt{broadcaster.broadcast()} is secured via one Mutex that locks.
    ...
    \item \textbf{Progress}
    Progress means that only the threads that execute the critical section should pay for its synchronization.
    We want to place the synchronization mechanism as narrow as possible.
    The broadcaster's global Mutex is too broad.
    It would be better to place the Lock more narrow, and only lock one map.
\end{enumerate}

\section{Proposed Solution to the Race Condition}
\label{proposedsolution}

To secure the critical section we could introduce a function \rustsnippet{lock()} that locks one map for read/write access.
...
It uses the Rust crate scc \cite{scccrate}, that comes with a concurrent hash map, that allows locking of individual entries.
...
Litt et al. presents Peritext, a Conflict-Free Replicated Data Type (CRDT) algorithm for collaborative rich text editing \cite{peritext}.
...
The computer used to perform the tests has the following specification:
...
To evaluate the performance of the event broadcasting mechanism in the PermaplanT Map Editor, we developed the Bash script \bashsnippet{test_perf.sh}.
...
The script outputs the slowest, the fastest times, as well as the average and median time of all responses.

\bashsnippet{test_perf.sh} tests the return times for the different amounts of connected users.
We simulate the response times with the following number of connected users: \texttt{10 20 50 100 150 200 500 1000 2000 5000 10000}.
...
\begin{enumerate}
    ...
    \item Simulate the required number of concurrent users by running \bashsnippet{connect_listener.py}, which establishes connections to the event broadcasting service.
...
The script \bashsnippet{test_order.py} is designed to test whether database writes and actions occur in the same order.
...
The \bashsnippet{test_order.py} script executes the following steps:
\begin{enumerate}
    \item Start the backend in release mode to ensure optimal performance.
    \item Sleep 5 seconds to allow the backend to fully stabilize, minimizing the impact of statup overhead.
    ...
    \item Execute \bashsnippet{connect_listener.py} with the \bashsnippet{--record_actions=true} flag, redirecting output to \bashsnippet{./target/actions.txt}. This records all actions of type \texttt{MovePlanting} to \texttt{actions.txt}.
...
We used classical UNIX utility \texttt{diff} \cite{unix-diff-paper} to compare both files line by line.
...
\subsection{Existent implementation}

The following shows the results of the performance of the existing (incorrect) implementation.
...
These are the request times for the suggested implementation of the broadcaster.
...
The proposed improvement from the Section \ref{proposedsolution} showed the same ordering between database writes and actions.

\chapter{Discussion}
This thesis evaluated the performance and correctness of PermaplanT's collaborative Map Editor, focusing on the scalability of the broadcasting mechanism and the presence of a race condition.
We identified that the asynchronous database update and the broadcaster form a critical section and suggested an alternative implementation of the broadcaster.

\section{Performance}

The performance of both the existing implementation and the suggested fix for the broadcasters race condition have similar performance characteristics.
he proposed solution using mutual exclusion did not significantly impact performance, as the locking mechanism was designed to be lightweight and map-specific.
...
Both test scripts can serve the project as helpful tools to optimize the broadcasting even further.
The tests allows us to measure how much the broadcasting mechanism slows down map update request in easily repeatable manner.
