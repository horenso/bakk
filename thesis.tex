...
\newcommand{\perfTableAndGraphFromCsv}[1]{
    ...
    \pgfplotstabletypeset[
        ...
        columns={Measuring,Fastest,Slowest,Average,Median},
...
The script \bashsnippet{test_order.py} is designed to verify whether database writes and actions occur in the same order.
...
\begin{enumerate}
    ...
    \item Simulate 100 concurrent users by running \bashsnippet{connection_listener.py}. This creates a high load on the system, increasing the likelihood of encountering the race condition.
...
The two resulting files \texttt{actions.txt} and \texttt{db.txt} are formatted in the same way, one update per line.
...
It simply means that, during the test, the actions and database writes happened to occur in the same order.

\chapter{Results}

\section{Performance Test}

\subsection{Exisint implementation}
...
In one test-run with 200 additional connections there were 34 action received in the wrong order.
